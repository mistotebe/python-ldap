                      ...
                      int add_intermediates)
{
    ...
    for (entry = ldap_first_entry(ld, m);
        ...
        if (dn == NULL) {
            Py_DECREF(result);
            ldap_msgfree(m);
            return LDAPerror(ld, "ldap_get_dn", NULL);
        }
        ...
        if (rc) {
            ...
            ldap_msgfree(m);
            ldap_memfree(dn);
            return LDAPerror(ld, "ldap_get_entry_controls", NULL);
        }
        ...
        if (!(pyctrls = LDAPControls_to_List(serverctrls))) {
            ...
            return LDAPerror(ld, "LDAPControls_to_List", NULL);
        }
        ldap_controls_free(serverctrls);
    ...
    for (entry = ldap_first_reference(ld, m);
        ...
        if (ldap_parse_reference(ld, entry, &refs, &serverctrls, 0) !=
            ...
            Py_DECREF(result);
            ldap_msgfree(m);
            return LDAPerror(ld, "ldap_parse_reference", NULL);
        }
        /* convert serverctrls to list of tuples */
        if (!(pyctrls = LDAPControls_to_List(serverctrls))) {
            int err = LDAP_NO_MEMORY;
        ...
        for (entry = ldap_first_message(ld, m);
            ...
            if (LDAP_RES_INTERMEDIATE == ldap_msgtype(entry)) {
                ...
                if (ldap_parse_intermediate
                    ...
                    Py_DECREF(result);
                    ldap_msgfree(m);
                    return LDAPerror(ld, "ldap_parse_intermediate", NULL);
                }
                /* convert serverctrls to list of tuples */
                if (!(pyctrls = LDAPControls_to_List(serverctrls))) {
                    int err = LDAP_NO_MEMORY;
